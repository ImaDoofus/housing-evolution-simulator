...
const hoveredCommandFitness = document.getElementById('hovered-command-fitness');
const hoveredCommandText = document.getElementById('hovered-command-text');
const hoveredCommandGeneration = document.getElementById('hovered-command-generation');
...
const simulationLikenessToTarget = document.getElementById('simulation-likeness-to-target');
...
function renderCanvases() {
	...
	ctx.fillStyle = command.block ? 'rgba(0, 100, 0, 0.8)' : 'rgba(0, 100, 0, 0.2)';
	ctx.fillRect(command.x1, command.z1, command.x2 - command.x1, command.z2 - command.z1);
}
...
function previewCommandOnCanvas(command) {
	...
	const ctx = currentCanvas.getContext('2d');
	ctx.fillStyle = command.block ? 'rgba(100, 0, 0, 0.8)' : 'rgba(100, 0, 0, 0.2)';
	ctx.fillRect(command.x1, command.z1, command.x2 - command.x1, command.z2 - command.z1);
}
...
executeCommand.onclick = () => {
	const command = population[0];
	currentTerrain.setRegion(command.x1, command.z1, command.x2, command.z2, command.block);
	renderCanvases();
	...
	const likenessPercent = currentTerrain.getLikeness(targetTerrain) / (TERRAIN_SIZE * TERRAIN_SIZE);
